...
import { assetToConfigurationMap } from './productConfiguration';
...
export class GeneralProductSeeder implements CustomResource {
	...
	private assembleCreateDataAssetTaskPayload(key: string, bucket: string): NewDataAssetTaskResource {
		...
		console.log(`${assetToConfigurationMap[assetName]} ----------`);
		...
		const newDataAssetTaskResource: NewDataAssetTaskResource = {
			...
			'workflow': {
				// This will include transform or data quality configuration if exists
				...assetToConfigurationMap[assetName],
