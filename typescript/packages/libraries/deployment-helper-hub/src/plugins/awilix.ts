...
import { RecipeProductSeeder } from '../seeders/recipeProduct';
...
declare module '@fastify/awilix' {
	interface Cradle {
		...
		recipeProductSeeder: RecipeProductSeeder;
	}
}
...
const sdfAdminUserId = process.env['SDF_ADMINISTRATOR_USER_ID'];
...
const dfRequestContext = getLambdaRequestContext(adminEmailAddress, sdfAdminUserId);
...
container.register({
	...
	customResourceManager: asFunction((container: Cradle) => new CustomResourceManager(logger, container.usepaProductSeeder, container.generalProductSeeder, container.redshiftProductSeeder, container.recipeProductSeeder), {
		...commonInjectionOptions
	}),
	...
	recipeProductSeeder: asFunction((container: Cradle) => new RecipeProductSeeder(logger, container.dataAssetClient, dataZoneMetadata, dfRequestContext), {
		...commonInjectionOptions
	}),
