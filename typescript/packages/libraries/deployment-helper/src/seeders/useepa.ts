...
import { getLambdaRequestContext } from '@df-sustainability/clients';
import type { DataZoneMetadata } from '../plugins/awilix';
import type { BaseLogger } from 'pino';

export class UsepaProductSeeder implements CustomResource {

  public constructor(private readonly log: BaseLogger, private readonly s3Client: S3Client, private readonly dataAssetClient: DataAssetClient, private readonly dataZoneMetadata: DataZoneMetadata) {
  }
  ...
  private async createAssets(bucket: string, prefix: string): Promise<void> {
    ...
    for (const object of listConvertedResponses.Contents) {
      if (object.Key.endsWith('csv') || object.Key.endsWith('xlsx')) {
        ...
        const year = assetName.split('-').pop();
        assetNameByYear[year] = assetName;
        createConvertedFutures.push(this.dataAssetClient.create(newDataAssetTaskResource, getLambdaRequestContext('sdf_demo', 'sdf_demo')));
      }
    }
