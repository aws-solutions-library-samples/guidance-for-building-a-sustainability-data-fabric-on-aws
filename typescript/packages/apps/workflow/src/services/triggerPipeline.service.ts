...
export class TriggerPipelineService {
	...
	constructor(private readonly log: BaseLogger,
				...
				private readonly glueRoleArn: string,
	...
	public async process(event: TriggerPipelineTask): Promise<[ExecutionDetailList, PipelineTask[]]> {
		...
		if (task.hasOwnProperty('sifResourceKey') && task.hasOwnProperty('resourceAssetName')) {
			...
			// Override the connector parameters with DataZone metadata
			const connectorOverrides = {
				...
				'sif-dataFabric-pipeline-output-connector': {
					parameters: {
						'domainId': this.domainId,
						'environmentId': this.environmentId,
						'roleArn': this.glueRoleArn,
						'projectId': this.projectId
					}
				}
			};
			await this.createDataFabricPipelineExecution(pipelineTask, connectorOverrides);
	...
	private async createDataFabricPipelineExecution(pipelineDataFabricTask: SinglePipelineDataFabricTask, connectorOverrides?: Record<string, ConnectorConfig>): Promise<ExecutionDetail | undefined> {

		this.log.info(`TriggerPipelineService > createPipelineExecution > pipelineDataFabricTask: ${pipelineDataFabricTask}, connectorOverrides: ${connectorOverrides}`);

		const createNewPipelineResource: NewPipeline = JSON.parse(await (await this.s3Client.send(new GetObjectCommand({ Bucket: this.bucketName, Key: pipelineDataFabricTask.sifResourceKey }))).Body!.transformToString());
		...
		if (existingPipeline) {
			...
			const execution = await this.executionClient.create(existingPipeline!.id, {
				actionType: 'create',
				mode: 'job',
				expiration: 600,
				connectorOverrides
