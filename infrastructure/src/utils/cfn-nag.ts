...
// TODO: refactor to library so products and demo can share
...
export function ruleRolePolicyWithWildcardResources(pattern: string, name: string, serviceName: string) : AddCfnNagItem {
  return {
    ...
    rules_to_suppress: [
      ruleToSuppressRolePolicyWithWildcardResources(name, serviceName),
    ],
  };
}

export function ruleRolePolicyWithWildcardResourcesAndHighSPCM(pattern: string, name: string, serviceName: string) : AddCfnNagItem {
  return {
    paths_endswith: [pattern],
    rules_to_suppress: [
      ruleToSuppressRolePolicyWithWildcardResources(name, serviceName),
      ruleToSuppressRolePolicyWithHighSPCM(name),
    ],
  };
}

export function ruleToSuppressCloudWatchLogEncryption() {
  return {
    id: 'W84',
    reason:
...
export function ruleToSuppressRolePolicyWithHighSPCM(name: string) {
  return {
    id: 'W76',
    reason: `Policy is generated by ${name} which resources SPCM for IAM policy document is higher than 25`,
  };
}

export function ruleToSuppressRolePolicyWithWildcardResources(name: string, serviceName: string) {
  return {
    id: 'W12',
    reason: `Policy is generated by ${name}, * ${serviceName} resources for read only access or IAM limitation.`,
  };
}

export function ruleToSuppressRolePolicyWithWildcardAction(name: string) {
  return {
    id: 'F4',
    reason: `Policy is generated by ${name}, this policy requires related actions in order to start/delete/update cloudformation stacks with many other services`,
  };
}
...
export const commonCdkNagRules = [
  ...
  },
  {
    id: 'AwsSolutions-L1',
    reason:
      'Need support AWS China regions, the latest NodeJs version in China regions is 18, and latest version in global regions is 20',
  },
];
